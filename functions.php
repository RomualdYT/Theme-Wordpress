...
if ( ! function_exists( 'the_words_setup' ) ) :
	...
	function the_words_setup() {
		...
		add_theme_support(
			'custom-background',
			apply_filters(
				'the_words_custom_background_args',
				array(
...
function the_words_scripts() {
	...
	wp_enqueue_style( 'the-words-google-fonts', wptt_get_webfont_url( $fonts_url ), array(), _S_VERSION );
		...
		wp_enqueue_style( 'slick', get_template_directory_uri() . '/assets/lib/slick/slick.css', array(), _S_VERSION );
		wp_enqueue_style( 'the-words-style', get_stylesheet_uri(), array(), _S_VERSION );
